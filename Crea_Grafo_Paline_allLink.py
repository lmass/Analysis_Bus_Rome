## Copyright (C) 2016  Luca Massarelli
    ...
    ## le paline sono i nodi, ogni palina e' unita ad un'altra se esiste almeno 
    ## un percorso che passa per entrambe
    ## prima di lanciarlo:
...
stopGraph = snap.TUNGraph.New()      #creo un nuovo grafo direzionato
    ...
    for i in range(0,len(res['risposta']['percorsi'])):                      #per ogni linea ciclo sui percorsi e mi leggo le fermate di ogni percorso
        ...
        if(go == 1):
            ...
            if(active == 1):                                                        #se e' attivo procedo ad aggiungere le fermate alla rete
                id_fermata = range(0,len(fermate['risposta']['fermate']))         #alloco spazio per la lista delle fermate
                ...
                for k in range(0,len(id_fermata)):                                         #ciclo su tutte le fermate della linea
                    for l in range(0,len(id_fermata)):
                        stopGraph.AddEdge(nodeTable[id_fermata[k]],nodeTable[id_fermata[l]])  #aggiungo link tra tutte le fermate del percorso
